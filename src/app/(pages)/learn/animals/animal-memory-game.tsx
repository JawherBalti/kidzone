...
const AnimalMemoryGame = ({ setIsPlayingAnimalMemoryGame, onClose }: any) => {
          ...
          <div className="flex flex-col-reverse lg:flex-row gap-3">
                    ...
                    <button
                      onClick={
                        pathname.includes("play")
                          ? () => onClose && onClose()
                          : () => setIsPlayingAnimalMemoryGame(false)
                      }
            ...
            {!showTips && (
              <div className="flex-1">
                <div className="relative flex justify-center items-center h-[100%] bg-white rounded-3xl p-6 shadow-2xl border-4 border-green-400">
                  {/* Fun Fact Popup */}
                  <AnimatePresence>
                    {showFact && (
                      <motion.div
                  ...
                  >
                    {gameState.cards.map((card) => (
                      <motion.div
                        key={card.id}
                        className={`${gameState.level === 3 && "w-[60%]"} ${
                          gameState.level === 2 && "w-[70%]"
                        } ${
                          gameState.level === 1 && "w-full"
                ...
                >
                  ...
                  <div className="bg-yellow-50 rounded-2xl p-4 mb-6 border-2 border-yellow-200">
                    ...
                    <div className="text-sm text-yellow-600 space-y-1">
                      <div>
                        Matches: {gameState.matches} × 100 ={" "}
                        {gameState.matches * 100}
                      </div>
                      <div>
                        Efficiency Bonus:{" "}
                        {Math.max(0, 500 - gameState.moves * 10)}
                      </div>
                      <div>Level Bonus: {gameState.level * 200}</div>
                    </div>
                  </div>
                  ...
                  >
                    {gameState.level < 3 ? "Next Level ➜" : "Finish Game! 🏆"}
                  </button>
                </motion.div>
              </motion.div>
            )}

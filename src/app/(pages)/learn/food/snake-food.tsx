...
export default function SnakeFoodGame({ setIsPlayingSnake }: SnakeGameProps) {
  ...
  const initializeLevel = (targetLevel: number) => {
    console.log(
      ...
      targetLevel,
      "with",
      LEVEL_CONFIG[targetLevel as keyof typeof LEVEL_CONFIG].foods,
      "foods"
    );
    ...
    console.log("🍎 Generating", config.foods, "foods for level", targetLevel);
    ...
    console.log("✅ Generated", newFoods.length, "foods");
    setFoods(newFoods);
  ...
  useEffect(() => {
    console.log("🚀 GAME STARTING - Level 1 (PAUSED)");
    initializeLevel(1);
  ...
  const startGame = () => {
    if (!gameStarted && isPaused) {
      console.log("🎮 Game starting with first key press!");
      setGameStarted(true);
      setIsPaused(false);
    }
  };
  ...
  const moveSnake = () => {
    ...
    if (foodWasEaten) {
      ...
      console.log(
        "🍽️ Food eaten! Snake should grow. New length:",
        newSnake.length
      );
      ...
      console.log("🗑️ Foods after removal:", newFoods.length);
      ...
      console.log("🚶 No food eaten. Snake maintains length:", newSnake.length);
    }

    console.log("🐍 Final snake length:", newSnake.length);
  ...
  useEffect(() => {
    if (
      ...
      console.log("🔄 Foods state changed, checking completion...");
  ...
  const restartGame = () => {
    console.log("🔄 RESTARTING GAME FROM LEVEL", level);
    setScore(0);
    setSuccess(false);
    setGameOver(false);
    setLevelComplete(false);
    ...
    <div className="w-full h-full bg-gradient-to-br from-sky-100 to-blue-100 rounded-3xl border-4 border-yellow-400">
      <div className="w-full h-full mx-auto p-5 px-3 flex flex-col md:flex-row justify-between gap-2 md:gap-8">
        ...
        <div className="md:w-[30%] text-center flex gap-3 flex-col rounded-3xl border-4 border-blue-400 bg-white p-4">
            ...
            <ul className="text-sm font-bold text-yellow-700 space-y-2">
              <li className="flex items-center gap-3">
                <span className="text-md">
                  Eat all food and aboid obstacles!
                </span>
              </li>
              <li className="flex items-center gap-3">
          ...
          <button
            onClick={() => setIsPlayingSnake(false)}
        ...
        <div className="flex-1 mx-auto h-full w-full">
          <div
            ref={gameAreaRef}
            tabIndex={0}
            className="h-full bg-white rounded-3xl shadow-2xl border-4 border-green-300 focus:outline-none relative"
          >
            {success ? (

...
const ShapePuzzleAdventure = ({ setIsPlayingShapePuzzle }: any) => {
  ...
  const handleDrag = (pieceId: number, event: any, info: any) => {
    ...
    if (target) {
      const distance = Math.sqrt(
        Math.pow(newX - target.targetX, 2) + Math.pow(newY - target.targetY, 2)
      );
    ...
    if (target) {
      ...
      if (distance < 60) {
        setGameState((prev) => {
          ...
          const placedPieces = updatedPieces.filter((p) => p.isPlaced).length;
          const allPiecesPlaced =
            placedPieces === prev.puzzles[prev.currentPuzzle].pieces.length;
  ...
  const progress = (gameState.placedPieces / currentPuzzle.pieces.length) * 100;
                ...
                <div className="space-y-4">
                  <div className="bg-yellow-50 p-4 rounded-2xl border-2 border-yellow-200">
                    ...
                    <ul className="text-sm text-yellow-700 space-y-1">
                      <li>• Drag shapes from bottom to dashed areas</li>
                      <li>• Match shapes to their outlines</li>
                      <li>
                        •{" "}
                      ...
                      <li>• Complete all pieces to win!</li>
                    </ul>
                  </div>
              ...
              <div className="flex-1 lg:col-span-2 w-full h-full">
                <div className="bg-white rounded-3xl p-6 shadow-2xl border-4 border-green-300 h-full">
                  ...
                  >
                    ...
                    {gameState.puzzles[gameState.currentPuzzle].pieces.map(
                      (target, index) => {
                        const isPlaced = gameState.pieces.some(
                          (p) =>
                            p.shape === target.shape &&
                            p.emoji === target.emoji &&
                            p.isPlaced
                        );
                        return !isPlaced ? (
                          <motion.div
                            ...
                            style={{
                              ...
                              left: target.targetX - (target.width + 20) / 2,
                              top: target.targetY - (target.height + 20) / 2,
                            }}
                            initial={{
                              opacity: 0,
                              scale: 0.8,
                    ...
                    {gameState.pieces.map((piece) => {
                      ...
                      const dragBorderColor = isDragging
                        ? getDragBorderColor(dragDistance)
                        : piece.isPlaced
                        ? "border-white"
                        : "border-yellow-400";

                      if (piece.shape.includes("Triangle")) {
                        return (
                          <motion.div
                            key={piece.id}
                            ...
                            onDrag={(event, info) =>
                              handleDrag(piece.id, event, info)
                            }
                            onDragEnd={(event, info) =>
                              handleDragEnd(piece.id, event, info)
                            }
                            whileDrag={{
                              scale: 1.1,
                              zIndex: 50,
                          ...
                          >
                            ...
                            <div
                              ...
                              style={{
                                borderLeftWidth: `${piece.width / 1.5}px`,
                                borderRightWidth: `${piece.width / 1.5}px`,
                                borderBottomWidth: `${piece.height}px`,
                                borderBottomColor: isDragging
                                  ? getDragColor(dragDistance).replace(
                                      "bg-",
                                      ""
                                    ) === "green-500"
                                    ? "#10B981"

...
const ShapeCarnivalGame = ({ setIsPlayingShapeCarnival, onClose }: any) => {
  ...
  const catchShape = (shape: any, binIndex: any) => {
    ...
    if (targetBin && targetBin.type === shape.type) {
      ...
      setGameState((prev: any) => ({
        ...
        fallingShapes: prev.fallingShapes.filter((s: any) => s.id !== shape.id),
    ...
    return (
      ...
      >
        ...
        <div className="flex gap-5 justify-center items-center">
          <button
            onClick={replay}
            className="px-8 py-4 bg-gradient-to-r from-pink-600 to-orange-400 text-white text-xl rounded-full font-bold hover:scale-105 transition-transform shadow-lg"
          >
            Play again
          </button>
          <button
            onClick={
              pathname.includes("play")
                ? () => onClose && onClose()
                : () => setIsPlayingShapeCarnival(false)
            }
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b from-purple-100 to-pink-100 p-3 md:p-6 rounded-3xl shadow-2xl border-4 border-yellow-400">
      <div className="w-full mx-auto flex flex-col md:flex-row justify-between gap-2 md:gap-8 h-full ">
        ...
        <div className="md:w-[40%] text-center flex flex-col">
          ...
          <div className="w-full text-center mt-2 md:mt-6 p-2 md:p-4 bg-yellow-50 rounded-2xl border-2 border-yellow-200">
            <p className="text-md md:text-lg text-yellow-800 font-semibold">
              💡 Click on falling shapes to catch them in the correct bins!
            </p>
          </div>
          <button
            onClick={
        ...
        {/* Game Area */}
        <div className="flex flex-col bg-white rounded-3xl p-0 md:p-8 shadow-2xl border-4 border-blue-400 md:w-[60%] h-full">
          {/* Falling Shapes Area */}
            ...
            <AnimatePresence>
              {gameState.fallingShapes.map((shape: any) => (
                <motion.div
                  ...
                  onClick={() => catchShape(shape, shape.position)}
                >
                  {shape.emoji}
                </motion.div>
              ))}
